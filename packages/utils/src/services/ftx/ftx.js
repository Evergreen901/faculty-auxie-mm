...
const Exchange = require("./pro/base/Exchange");
...
module.exports = class ftx extends Exchange {
  describe() {
    return this.deepExtend(super.describe(), {
      ...
      certified: false,
      pro: true,
      hostname: "host.docker.internal:8088", // or ftx.us
      urls: {
    ...
    for (let i = 0; i < markets.length; i++) {
      ...
      if (swap) {
        ...
        if (expiry === undefined) {
          ...
          const throwOnUndefinedExpiry = this.safeValue(
            options,
            "throwOnUndefinedExpiry",
            false
          );
          if (throwOnUndefinedExpiry) {
            throw new BadResponse(
              this.id +
                " symbol '" +
                id +
                "' is a future contract with an invalid expiry datetime."
            );
          } else {
            continue;
          }
        ...
        symbol =
          base + "/" + quote + ":" + settle + "-" + this.yymmdd(expiry, "");
      }
      ...
      if (minProvideSize !== undefined) {
        minAmountString = Precise.stringGt(minProvideSize, sizeIncrement)
          ? sizeIncrement
          : minProvideSize;
      }
      result.push({
  ...
  async fetchTicker(symbol, params = {}) {
    ...
    const response = await this.publicGetMarketsMarketName(
  ...
  async fetchOrderBook(symbol, limit = undefined, params = {}) {
    ...
    const response = await this.publicGetMarketsMarketNameOrderbook(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchOHLCV(
     ...
     */
    await this.loadMarkets();
    const [market, marketId] = this.getMarketParams(
      symbol,
      "market_name",
      params
    );
    ...
    if (since === undefined) {
      // Issue #12855, inconsistent results if since is not provided
  ...
  async fetchTrades(symbol, since = undefined, limit = undefined, params = {}) {
    ...
    const response = await this.publicGetMarketsMarketNameTrades(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchFundingRateHistory(
    ...
    const response = await this.publicGetFundingRates(
      this.extend(request, params)
    );
    //
    //     {
  ...
  parseOrder(order, market = undefined) {
    ...
    const lastTradeTimestamp = this.parse8601(
      this.safeString(order, "triggeredAt")
    );
  ...
  async createOrder(
    ...
    if (isStopOrder) {
      if (triggerPrice === undefined) {
        if (isTakeProfit) {
          throw new ArgumentsRequired(
            this.id +
              " createOrder() requires a triggerPrice param, a stopPrice or a takeProfitPrice param for a takeProfit order"
          );
        } else if (isStopLoss) {
          throw new ArgumentsRequired(
            this.id +
              " createOrder() requires a triggerPrice param, a stopPrice or a stopLossPrice param for a stop order"
          );
        }
      }
      method = "privatePostConditionalOrders";
      request["triggerPrice"] = parseFloat(
        this.priceToPrecision(symbol, triggerPrice)
      );
      if (type === "limit" && price === undefined) {
        throw new ArgumentsRequired(
          this.id +
            " createOrder () requires a price argument for stop limit orders"
        );
      }
      if (price !== undefined) {
        request["orderPrice"] = parseFloat(
      ...
      if (timeInForce !== undefined) {
        if (!(timeInForce === "IOC" || timeInForce === "PO")) {
          throw new InvalidOrder(
            this.id +
              " createOrder () does not accept timeInForce: " +
              timeInForce +
              " orders, only IOC and PO orders are allowed"
          );
        }
      }
      ...
      if (trailValue === undefined) {
        throw new ArgumentsRequired(
          this.id +
            " createOrder () requires a trailValue parameter or a price argument (negative or positive) for a " +
            type +
            " order"
        );
      }
      method = "privatePostConditionalOrders";
      request["trailValue"] = parseFloat(
        this.priceToPrecision(symbol, trailValue)
  ...
  async editOrder(
    ...
    if (triggerPriceIsDefined || orderPriceIsDefined || trailValueIsDefined) {
      ...
      if (orderPriceIsDefined) {
        ...
        request["orderPrice"] = parseFloat(
          this.priceToPrecision(symbol, orderPrice)
        );
      }
  ...
  async cancelOrder(id, symbol = undefined, params = {}) {
    ...
    if (clientOrderId === undefined) {
      ...
      if (
        isTriggerOrder ||
        this.inArray(type, ["stop", "trailingStop", "takeProfit"])
      ) {
  ...
  async fetchOpenOrders(
    ...
    if (
      isTriggerOrder ||
      this.inArray(type, ["trigger", "stop", "trailingStop", "takeProfit"])
    ) {
  ...
  async fetchOrderIfFromConditionalOrder(id) {
    ...
    const response =
      await this.privateGetConditionalOrdersConditionalOrderIdTriggers(request);
  ...
  async transfer(code, amount, fromAccount, toAccount, params = {}) {
    ...
    const response = await this.privatePostSubaccountsTransfer(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async withdraw(code, amount, address, tag = undefined, params = {}) {
    ...
    const response = await this.privatePostWalletWithdrawals(
      this.extend(request, params)
    );
    //
    //     {
  ...
  parsePosition(position, market = undefined) {
    ...
    const liquidationPriceString = this.safeString(
      position,
      "estimatedLiquidationPrice"
    );
    const initialMarginPercentage = this.safeString(
      position,
    ...
    const maintenanceMarginPercentageString = this.safeString(
      position,
    ...
    const percentage = this.parseNumber(
      Precise.stringMul(
        Precise.stringDiv(unrealizedPnlString, initialMargin, 4),
        "100"
      )
    );
    ...
    if (Precise.stringEq(liquidationPriceString, "0")) {
      ...
      leverage = this.parseNumber(
        Precise.stringDiv(
          Precise.stringAdd(
            Precise.stringDiv(notionalString, collateral),
            "0.005"
          ),
          "1",
          2
      ...
      marginRatio = this.parseNumber(
        Precise.stringDiv(maintenanceMarginString, collateral, 4)
      );
    }
  ...
  async fetchDepositAddress(code, params = {}) {
    ...
    const response = await this.privateGetWalletDepositAddressCoin(
      this.extend(request, params)
    );
    //
    //     {
  ...
  sign(
    ...
    const chosenSubaccount = this.safeString2(
      params,
      subaccountField,
      "subaccount"
    );
  ...
  async setLeverage(leverage, symbol = undefined, params = {}) {
    ...
    if (leverage < 1 || leverage > 20) {
      throw new BadRequest(
        this.id + " setLeverage () leverage should be between 1 and 20"
      );
    }
    const request = {
      leverage: leverage,
  ...
  async fetchFundingHistory(
    ...
    const response = await this.privateGetFundingPayments(
      this.extend(request, params)
    );
    const result = this.safeValue(response, "result", []);
  ...
  async fetchFundingRate(symbol, params = {}) {
    ...
    const response = await this.publicGetFuturesFutureNameStats(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchBorrowRateHistories(
    codes = undefined,
    since = undefined,
    limit = undefined,
    params = {}
    ...
    if (numCodes === 1) {
      ...
      if (limit !== undefined && limit > 5000) {
        throw new BadRequest(
          this.id +
            " fetchBorrowRateHistories () limit cannot exceed 5000 for a single currency"
        );
      }
      if (
        endTime !== undefined &&
        since !== undefined &&
        endTime - since > millisecondsPer5000Hours
      ) {
        throw new BadRequest(
          this.id +
            " fetchBorrowRateHistories () requires the time range between the since time and the end time to be less than 5000 hours for a single currency"
        );
      }
      ...
      if (limit !== undefined && limit > 48) {
        throw new BadRequest(
          this.id +
            " fetchBorrowRateHistories () limit cannot exceed 48 for multiple currencies"
        );
      }
      if (
        endTime !== undefined &&
        since !== undefined &&
        endTime - since > millisecondsPer2Days
      ) {
        throw new BadRequest(
          this.id +
            " fetchBorrowRateHistories () requires the time range between the since time and the end time to be less than 48 hours for multiple currencies"
        );
      }
    }
    ...
    const response = await this.publicGetSpotMarginHistory(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async fetchBorrowRateHistory(
     ...
     */
    const histories = await this.fetchBorrowRateHistories(
      [code],
      since,
      limit,
      params
    ...
    if (borrowRateHistory === undefined) {
      throw new BadRequest(
        this.id + " fetchBorrowRateHistory () returned no data for " + code
      );
    }
  ...
  parseBorrowRateHistories(response, codes, since, limit) {
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      if (codes === undefined || this.inArray(code, codes)) {
        ...
        const borrowRate = Precise.stringMul(
          lendingRate,
          Precise.stringAdd("1", spotMarginBorrowRate)
        );
        const borrowRateStructure = this.extend(this.parseBorrowRate(item), {
          rate: borrowRate,
        });
    ...
    for (let i = 0; i < keys.length; i++) {
      const code = keys[i];
      borrowRateHistories[code] = this.filterByCurrencySinceLimit(
        borrowRateHistories[code],
        code,
        since,
        limit
  ...
  async fetchBorrowInterest(
    ...
    const response = await this.privateGetSpotMarginBorrowHistory(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchOpenInterest(symbol, params = {}) {
    ...
    if (!market["contract"]) {
      throw new BadRequest(
        this.id + " fetchOpenInterest() supports contract markets only"
      );
    }
    const request = {
      future_name: market["id"],
