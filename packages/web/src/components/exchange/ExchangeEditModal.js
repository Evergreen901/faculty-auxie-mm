...
const ExchangeEditModal = () => {
  ...
  useEffect(() => {
    if (!gatherer || !gatherer.exchanges) return;

    if (mounted.current) setData(JSON.stringify(gatherer.exchanges, null, 2));
  }, [gatherer]);

  const onSave = async () => {
    if (!gatherer) return;
    try {
      gatherer.exchanges = JSON.parse(data);
    } catch (ex) {
    ...
    toast.success('Updated exchanges');
    getServices();
    closeModal('exchangeEdit');
  };

  return (
    <Modal open={modal.exchangeEdit} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
      <Box
        sx={{
          position: 'absolute',
          top: '50%',
      ...
      >
        ...
        >
          <IconButton
            ...
            onClick={() => closeModal('exchangeEdit')}
          >
            <XIcon />
          </IconButton>
